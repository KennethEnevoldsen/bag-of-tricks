...
def jsonl_merge(jsonl_files: List[Union[Path, str]], buffer_size: Optional[int] = None, sample: bool=True) -> Iterable[dict]:
    """
    ...
    if buffer_size:
        json_gen = shuffle_buffer(jsonl_merge(jsonl_files=jsonl_files, buffer_size = None, sample=sample), buffer_size=buffer_size)
